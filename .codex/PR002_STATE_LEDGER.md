...
- Real pinned TKG integration test: `PASS` locally AND in a clean-clone repro — `vitest.integration.config.ts` → `src/book-skill/integration/tkg-pipeline.int.ts`: pinned source acquired+verified (`052049f4`, `VERIFIED_MIT`, frozen contract files present), real `createBookSkillLlmClient` via the unified gateway against a local deterministic OpenAI-compatible endpoint, real pipeline wrote the upstream layout (SKILL.md, chapters/, chapters_manifest.json, raw/), and the upstream `lint_chapters.py --strict` gate passed with `0 warning(s)`. First run honestly FAILED the lint gate (192-word toolkits below the 200-word band) and was fixed by enriching the deterministic fixture — the gate is real, not decorative.
- CI round 1 on `6be7ee8c`: blocking quality PASS, blocking Read-Box integration PASS, blocking TKG integration **FAIL**, non-gating baseline debt FAIL (expected). Root cause found via minimal probe tests in a clean local clone: the integration test imported `scripts/tkg-source.mjs` from outside the vitest package root, and vitest cannot reliably evaluate such modules (`SyntaxError: Invalid or unexpected token` at suite load; the workflow's native `node scripts/tkg-source.mjs ensure` step itself PASSED). Fix: the test no longer imports the script; it computes the checkout dir (env-overridable `TKG_CHECKOUT_DIR`), re-verifies the pin read-only via `git -C <dir> rev-parse HEAD`, and cross-checks the pinned ref against the script's constant by text so the two declaration sites cannot drift. Fix verified in the clean clone (lint gate 0 warnings) before push.
